...
function extractStats(result) {
  ...
  const stats = {
    ...
    views: result.statistics?.playCount || result.stats?.playCount || result.playCount || result.statistics?.viewCount || result.stats?.viewCount || 0,
...
function transformLibraryResponse(libraryData) {
  ...
  const thumbnail = result.cover?.[0] || result.originCover?.[0] || result.video?.cover?.[0] || result.video?.originCover?.[0] || result.dynamicCover?.[0] || result.video?.dynamicCover?.[0] || null;
  ...
  if (result.author?.avatarLarger) {
    avatar = Array.isArray(result.author.avatarLarger) ? result.author.avatarLarger[0] : result.author.avatarLarger;
  }
  if (!avatar && result.author?.avatarMedium) {
    avatar = Array.isArray(result.author.avatarMedium) ? result.author.avatarMedium[0] : result.author.avatarMedium;
  }
  ...
  if (!avatar && result.author?.avatar) {
    avatar = Array.isArray(result.author.avatar) ? result.author.avatar[0] : result.author.avatar;
  }
  ...
  return {
    ...
    result: {
      ...
      uploadDate: result.createTime ? new Date(result.createTime * 1e3).toISOString() : null,
...
async function tryLibraryDownloader(url) {
...
async function fallbackToExternalServices(url) {
  const services = [
    {
      name: "TikWM",
      ...
      transform: (data) => {
        ...
        const avatar = data.data?.author?.avatar_thumb?.url_list?.[0] || data.data?.author?.avatar_larger?.url_list?.[0] || data.data?.author?.avatar_medium?.url_list?.[0] || data.data?.author?.avatar || null;
        ...
        return {
          ...
          result: {
            ...
            uploadDate: data.data?.create_time ? new Date(data.data.create_time * 1e3).toISOString() : null,
...
const GET = async ({ request, url: requestUrl }) => {
  const headers = {
